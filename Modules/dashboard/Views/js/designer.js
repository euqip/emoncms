...
var designer = {
    ...
    'draw_options': function(widget)
    {
        ...
        var helptext = (widgets[widget].helptext=== undefined) ? '' : widgets[widget].helptext;
        var i,z, selected;
        ...
        for (z in box_options)        {
            ...
            html += "<tr><td class = 'option_name'>"+options_name[z]+":</td>";
            if (options_type){
                switch (options_type[z]){
                    ...
                    case "feedid":
                        html += "<td><select id='"+box_options[z]+"' class='form-control options' >";
                        for (i in feedlist) {
                            selected =  (val === feedlist[i].id)? 'selected' : '';
                            html += "<option value='"+feedlist[i].id+"' "+selected+" >"+feedlist[i].id+": "+feedlist[i].name+"</option>";
                        }
                        break;
                    case "multigraph":
                        html += "<td><select id='"+box_options[z]+"' class='form-control options' >";
                        for (i in multigraphs)                {
                            selected =  (val === multigraphs[i].id)? 'selected' : '';
                            html += "<option value='"+multigraphs[i].id+"' "+selected+" >"+multigraphs[i].id+": "+multigraphs[i].name+"</option>";
                        }
                        break;
                    case "html":
                    ...
                    case "dropbox":
                        if ( optionsdata[z]){
                            ...
                            for (i in optionsdata[z])       {
                                selected =  (val === optionsdata[z][i][0])? 'selected' : '';
                                html += "<option "+selected+" value=\""+optionsdata[z][i][0]+"\">"+optionsdata[z][i][1]+"</option>";
                            }
                        }
                        break;
                    case "colour_picker":
            ...
            html += "</td><td><small><p class='muted'>"+optionshint[z]+"</p></small></td></tr>";
        }
        var x = 1/0;
    ...
    'widget_options': function(){
        if (designer.selected_box){
            var whichtype = $("#"+designer.selected_box).attr("class");
            var sel =whichtype.split(" ");
            //use only the first class element
                            //preventDefault
            $('#msgcontent').html(designer.draw_options(sel[0]));
     ...
     'widget_buttons': function()    {
        ...
        for (z in widgets) {
            if (widgets.hasOwnProperty(z)){
                menu = widgets[z].menu;
                displayname = (widgets[z].itemname===undefined)?z:widgets[z].itemname;
                if (typeof select[menu] === "undefined") select[menu]="";
                select[menu] += "<li><a id='"+z+"' class='widget-button'>"+displayname+"</a></li>";
            }
        }
        for (z in select){
            if (select.hasOwnProperty(z)){
    ...
    'add_widget': function(mx,my,type)    {
        ...
        $("#page").append('<div id="'+designer.boxi+'" class="'+type+' emon_widget" style="position:absolute; margin: 0; top:'+designer.snap(my+widgets[type].offsety)+'px; left:'+designer.snap(mx+widgets[type].offsetx)+'px; width:'+widgets[type].width+'px; height:'+widgets[type].height+'px;" >'+html+'</div>');
    ...
    'unsurround' : function(boxid){
        ...
        if (mybox.attr('class')!==undefined){
            ...
            var whichtype = mybox.attr('class');
            var sel =whichtype.split(" ");
            //use only the first class element
                            //preventDefault
            //$('#msgcontent').html(designer.draw_options(sel[0]));
            var fname = sel[0]+"_repaint";
